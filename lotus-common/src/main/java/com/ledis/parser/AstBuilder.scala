...
import com.ledis.catalog.table.TableCatalog._
import com.ledis.exception.AnalysisException
import com.ledis.expressions.aggregate.{First, Last}
import com.ledis.parser.SqlBaseParser._
import com.ledis.analysis._
import com.ledis.catalog.table.TableChange.ColumnPosition
import com.ledis.catalog.{BucketSpec, CatalogStorageFormat, FunctionResource, FunctionResourceType}
import com.ledis.config.SQLConf
import com.ledis.connector._
import com.ledis.expressions._
import com.ledis.expressions.expression._
import com.ledis.expressions.group.{Cube, Rollup}
...
import com.ledis.utils.util.{CharVarcharUtils, IntervalUtils}
...
class AstBuilder extends SqlBaseBaseVisitor[AnyRef] with SQLConfHelper {
  ...
  override def visitPartitionSpec(
    ...
    val legacyNullAsString =
      conf.getConf(SQLConf.LEGACY_PARSE_NULL_PARTITION_SPEC_AS_STRING_LITERAL).asInstanceOf[Boolean]
  ...
  private def withQueryResultClauses(
    ...
    val withOrder = if (
      !ctx.order.isEmpty && ctx.sort.isEmpty && ctx.distributeBy.isEmpty && ctx.clusterBy.isEmpty) {
      // ORDER BY ...
      Sort(ctx.order.asScala.map(visitSortItem).toSeq, global = true, query)
    } else if (ctx.order.isEmpty && !ctx.sort.isEmpty && ctx.distributeBy.isEmpty && ctx.clusterBy.isEmpty) {
      // SORT BY ...
      Sort(ctx.sort.asScala.map(visitSortItem).toSeq, global = false, query)
    } else if (ctx.order.isEmpty && ctx.sort.isEmpty && !ctx.distributeBy.isEmpty && ctx.clusterBy.isEmpty) {
      // DISTRIBUTE BY ...
      withRepartitionByExpression(ctx, expressionList(ctx.distributeBy), query)
    } else if (ctx.order.isEmpty && !ctx.sort.isEmpty && !ctx.distributeBy.isEmpty && ctx.clusterBy.isEmpty) {
      // SORT BY ... DISTRIBUTE BY ...
      Sort(
        ctx.sort.asScala.map(visitSortItem).toSeq,
        global = false,
        withRepartitionByExpression(ctx, expressionList(ctx.distributeBy), query))
    } else if (ctx.order.isEmpty && ctx.sort.isEmpty && ctx.distributeBy.isEmpty && !ctx.clusterBy.isEmpty) {
      // CLUSTER BY ...
      val expressions = expressionList(ctx.clusterBy)
      Sort(
    ...
    // WINDOWS
    val withWindow = withOrder.optionalMap(ctx.windowClause)(withWindowClause)

    // LIMIT
    // - LIMIT ALL is the same as omitting the LIMIT clause
    withWindow.optional(ctx.limit) {
      Limit(typedVisit(ctx.limit), withWindow)
    }
  }
  ...
  private def withSelectQuerySpecification(
    ...
    val withProject = if (aggregationClause == null && havingClause != null) {
      if (conf.getConf(SQLConf.LEGACY_HAVING_WITHOUT_GROUP_BY_AS_WHERE).asInstanceOf[Boolean]) {
   ...
   * either combined by a logical [[com.ledis.expressions.predicate.And]] or a logical [[com.ledis.expressions.predicate.Or]].
   *
  ...
  /**
   * Create an [[com.ledis.expressions.order.LambdaFunction]].
   */
  ...
  override def visitTinyIntLiteral(ctx: TinyIntLiteralContext): Literal = {
    ...
    numericLiteral(ctx, rawStrippedQualifier,
      Byte.MinValue.asInstanceOf[BigDecimal], Byte.MaxValue.asInstanceOf[BigDecimal], ByteType.simpleString)(_.toByte)
  }
  ...
  override def visitSmallIntLiteral(ctx: SmallIntLiteralContext): Literal = {
    val rawStrippedQualifier = ctx.getText.substring(0, ctx.getText.length - 1)
    numericLiteral(ctx, rawStrippedQualifier,
      Short.MinValue.asInstanceOf[BigDecimal], Short.MaxValue.asInstanceOf[BigDecimal], ShortType.simpleString)(_.toShort)
  }
  ...
  override def visitFloatLiteral(ctx: FloatLiteralContext): Literal = {
    val rawStrippedQualifier = ctx.getText.substring(0, ctx.getText.length - 1)
    numericLiteral(ctx, rawStrippedQualifier,
      Float.MinValue.asInstanceOf[BigDecimal], Float.MaxValue.asInstanceOf[BigDecimal], FloatType.simpleString)(_.toFloat)
  }
  ...
  override def visitComplexColType(ctx: ComplexColTypeContext): StructField = withOrigin(ctx) {
    val structField = StructField(
      ...
      nullable = NULL == null)
    Option(ctx.commentSpec).map(visitCommentSpec).map(structField.withComment).getOrElse(structField)
  }
  ...
  private def cleanNamespaceProperties(
    ...
    val legacyOn = conf.getConf(SQLConf.LEGACY_PROPERTY_NON_RESERVED).asInstanceOf[Boolean]
    properties.filter {
