...
import com.ledis.expressions.predicate.EqualNullSafe
...
abstract class LogicalPlan
   ...
   * [[com.ledis.analysis.UnresolvedRelation UnresolvedRelation]]
   * should return `false`).
   */
  ...
  protected def getAllValidConstraints(projectList: Seq[NamedExpression]): ExpressionSet = {
    ...
    projectList.foreach {
      ...
      case a @ Alias(e, _) =>
        ...
        allConstraints.baseSet ++= allConstraints.baseSet.map(
          _ transform {
           case expr: Expression if expr.semanticEquals(e) =>
            a.toAttribute
          }
        )
        allConstraints.originals ++= allConstraints.originals.map(
          _ transform {
            case expr: Expression if expr.semanticEquals(e) =>
              a.toAttribute
          }
