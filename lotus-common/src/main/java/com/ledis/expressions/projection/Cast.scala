...
abstract class CastBase extends UnaryExpression with TimeZoneAwareExpression with NullIntolerant {
  ...
  private val legacyCastToStr = SQLConf.get.getConf(SQLConf.LEGACY_COMPLEX_TYPES_TO_STRING).asInstanceOf[Boolean]
  ...
  private[this] def cast(from: DataType, to: DataType): Any => Any = {
    ...
    if (DataType.equalsStructurally(from, to)) {
      ...
      _ => throw new Exception(s"should not directly cast from NullType to $to.")
    } else {
      to match {
 ...
 * session local timezone by an analyzer [[com.ledis.analysis.ResolveTimeZone]].
 */
