...
import com.ledis.expressions.aggregate.{UnsafeArrayWriter, UnsafeRowWriter, UnsafeWriter}
import com.ledis.expressions.expression.{Expression, Nondeterministic, Stateful}
import com.ledis.types._
import com.ledis.utils.SpecializedGetters
import com.ledis.utils.collections.row.{GenericInternalRow, InternalRow, UnsafeRow}
import com.ledis.utils.collections.{UnsafeArrayData, UnsafeMapData}
...
object InterpretedUnsafeProjection {
   ...
   * or array) to the given buffer using the given [[com.ledis.expressions.aggregate.UnsafeWriter]].
   */
  private def generateFieldWriter(
    ...
    val unsafeWriter: (SpecializedGetters, Int) => Unit = dt match {
      ...
      case _ =>
        throw new Exception(s"Unsupported data type $dt")
    }
