...
import com.ledis.catalog.util.ExternalCatalogUtils
import com.ledis.exception._

import scala.collection.mutable
import com.ledis.catalog.util.ExternalCatalogUtils._
  ...
  override def dropDatabase(
    ...
    if (catalog.contains(db)) {
      ...
      try {
        ...
        case e: IOException =>
          throw new Exception(s"Unable to drop database ${dbDefinition.name} as failed " +
      ...
      if (tableMeta.tableType == CatalogTableType.MANAGED) {
        ...
        locationAllParts.foreach { loc =>
          ...
          try {
            ...
            case e: IOException =>
              throw new Exception(s"Unable to delete partition path $partitionPath", e)
          }
        }
        ...
        try {
          ...
          case e: IOException =>
            throw new Exception(s"Unable to drop table $table as failed " +
  ...
  override def renameTable(
    ...
    if (oldDesc.table.tableType == CatalogTableType.MANAGED) {
      ...
      try {
        ...
        case e: IOException =>
          throw new Exception(s"Unable to rename table $oldName to $newName as failed " +
  ...
  override def createPartitions(
    ...
    parts.foreach { p =>
      ...
      try {
        ...
        case e: IOException =>
          throw new Exception(s"Unable to create partition path $partitionPath", e)
      }
    ...
    specs.zip(newSpecs).foreach { case (oldSpec, newSpec) =>
      ...
      val newPartition = if (shouldUpdatePartitionLocation) {
        ...
        try {
          ...
          case e: IOException =>
            throw new Exception(s"Unable to rename partition path $oldPartPath", e)
        }
        oldPartition.copy(
          spec = newSpec,
