...
class SessionCatalog(
  ...
  protected def makeFunctionExpression(
    ...
    val clsForUDAF =
      Class.forName("com.ledis.sql.expressions.UserDefinedAggregateFunction")
    if (clsForUDAF.isAssignableFrom(clazz)) {
      val cls = Class.forName("com.ledis.execution.aggregate.ScalaUDAF")
