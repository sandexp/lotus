...
package com.ledis.utils.collections.row;
 ...
 */
public final class UnsafeRow extends InternalRow implements Externalizable, Serializable {
  ...
  public void setDecimal(int ordinal, Decimal value, int precision) {
    ...
    if (precision <= Decimal.MAX_LONG_DIGITS()) {
      ...
      if (value == null || !value.changePrecision(precision, value.scale())) {
        ...
        Platform.copyMemory(
          bytes, BYTE_ARRAY_OFFSET, baseObject, baseOffset + cursor, bytes.length);
  ...
  public void copyFrom(UnsafeRow row) {
    ...
    assert (baseObject instanceof byte[]) && baseOffset == BYTE_ARRAY_OFFSET;
  ...
  public void writeToStream(OutputStream out, byte[] writeBuffer) throws IOException {
    if (baseObject instanceof byte[]) {
      int offsetInByteArray = (int) (baseOffset - BYTE_ARRAY_OFFSET);
      ...
      while (dataRemaining > 0) {
        ...
        Platform.copyMemory(
          baseObject, rowReadPosition, writeBuffer, BYTE_ARRAY_OFFSET, toTransfer);
  ...
  public void writeFieldTo(int ordinal, ByteBuffer buffer) {
    ...
    Platform.copyMemory(
      ...
      baseOffset + offset,
      buffer.array(),
      BYTE_ARRAY_OFFSET + buffer.arrayOffset() + pos,
      size);
  }
