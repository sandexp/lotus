...
case class AccumulatorMetadata(
    id: Long,
    name: Option[String],
...
abstract class AccumulatorV2[IN, OUT] extends Serializable {
  ...
  final def isAtDriverSide: Boolean = atDriverSide
...
class CollectionAccumulator[T] extends AccumulatorV2[T, java.util.List[T]] {
  ...
  def setValue(newValue: java.util.List[T]): Unit = this.synchronized {
    _list = null
