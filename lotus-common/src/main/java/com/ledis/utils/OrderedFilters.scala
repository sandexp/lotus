...
import com.ledis.expressions.predicate.BasePredicate
 ...
 */
class OrderedFilters(filters: Seq[Filter], requiredSchema: StructType)
  ...
  private val predicates: Array[BasePredicate] = {
    ...
    val groupedFilters = Array.fill(len)(Seq.empty[Filter])
    for (filter <- filters) {
