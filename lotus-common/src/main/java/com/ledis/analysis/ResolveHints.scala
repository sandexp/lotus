...
import com.ledis.expressions.order.{Ascending, SortOrder}
import com.ledis.expressions.projection.IntegerLiteral
      ...
      val newNode = CurrentOrigin.withOrigin(plan.origin) {
        plan match {
          case ResolvedHint(u @ UnresolvedRelation(ident, _, _), hint)
            ...
            ResolvedHint(u, createHintInfo(hintName).merge(hint, null))

          case ResolvedHint(r: SubqueryAlias, hint)
              if matchedIdentifierInHint(extractIdentifier(r)) =>
            ResolvedHint(r, createHintInfo(hintName).merge(hint, null))
