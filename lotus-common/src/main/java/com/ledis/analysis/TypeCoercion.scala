...
import com.ledis.expressions.order.MapZipWith
import com.ledis.expressions.predicate._
import com.ledis.expressions.projection.{AnsiCast, Cast, Literal}
...
object TypeCoercion {
   ...
   */
  def findWiderTypeWithoutStringPromotionForTwo(
      t1: DataType,
  ...
  object ImplicitTypeCasts extends TypeCoercionRule {
    ...
    private def implicitCast(inType: DataType, expectedType: AbstractDataType): Option[DataType] = {
      ...
      val ret: DataType = (inType, expectedType) match {
