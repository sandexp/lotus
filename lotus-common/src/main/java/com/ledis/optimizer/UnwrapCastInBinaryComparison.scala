...
import com.ledis.expressions.expression.{Expression, IsNotNull, IsNull}
...
object UnwrapCastInBinaryComparison extends Rule[LogicalPlan] {
  ...
  def getRange(dt: DataType): Option[(Any, Any)] = dt match {
   ...
   */
  def falseIfNotNull(e: Expression): Expression = {
   ...
   */
  def trueIfNotNull(e: Expression): Expression = {
