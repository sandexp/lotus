 ...
 *
 * {{{
 *  scala> import com.ledis.dsl.expressions._
 *
 *  // Standard operators are added to expressions.
 *  scala> import com.ledis.expressions.Literal
 *  scala> Literal(1) + Literal(1)
 *  res0: com.ledis.expressions.Add = (1 + 1)
 *
 *  // There is a conversion from 'symbols to unresolved attributes.
 *  scala> 'a.attr
 *  res1: com.ledis.analysis.UnresolvedAttribute = 'a
 *
 *  // These unresolved attributes can be used to create more complicated expressions.
 *  scala> 'a === 'b
 *  res2: com.ledis.expressions.EqualTo = ('a = 'b)
 *
 *  // SQL verbs can be used to construct logical query plans.
 *  scala> import com.ledis.plans.logical._
 *  scala> import com.ledis.dsl.plans._
 *  scala> LocalRelation('key.int, 'value.string).where('key === 1).select('value).analyze
 *  res3: com.ledis.plans.logical.LogicalPlan =
 *  Project [value#3]
 *   Filter (key#2 = 1)
...
package object dsl {
  ...
  trait ExpressionConversions {
    ...
    implicit def symbolToUnresolvedAttribute(s: Symbol): com.ledis.analysis.UnresolvedAttribute =
      analysis.UnresolvedAttribute(s.name)

    /** Converts $"col name" into an [[com.ledis.analysis.UnresolvedAttribute]]. */
    implicit class StringToAttributeConversionHelper(val sc: StringContext) {
      ...
      def $(args: Any*): com.ledis.analysis.UnresolvedAttribute = {
